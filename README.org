...
- This =README.org= tangles to =init.el=. To build it, run =emacs -q --batch --eval '(org-babel-tangle-file "README.org")'= (this also produces =early-init.el=), then start the notebook with =emacs --init-directory .=. A pre-tangled =init.el= is committed for convenience.
...
#+begin_src R :session test :exports both :results output graphics file :file graphs/scatter.png :tangle no
...
#+BEGIN_SRC R :session test :exports both :results output :tangle no
library(tableone)
...
#+BEGIN_SRC R :var tab=with-rownames :rownames yes :tangle no
print(tab)
#+END_SRC
...
#+cite_export: csl chicago-fullnote.csl
...
#+latex_header: \institute{Your Institute \& \\ Your Department}
#+LATEX_HEADER: \usepackage{roboto}
#+LATEX_HEADER: \usepackage{tabularx}
#+LATEX_HEADER: \usepackage{listings}
#+LATEX_HEADER: \usepackage{xcolor}
#+LATEX_HEADER: \lstset{basicstyle=\ttfamily\tiny,keywordstyle=\color{blue},commentstyle=\color{gray},stringstyle=\color{orange},showstringspaces=false,breaklines=true,frame=none}
#+LATEX_HEADER: \usepackage{appendixnumberbeamer}
#+LATEX_HEADER: \usepackage{embedfile}
#+LATEX_HEADER: \usepackage{fvextra}
#+LATEX_HEADER: \embedfile[afrelationship={/Source}]{presentation.org}
#+LATEX_HEADER: \usepackage[font={tiny}]{caption}
#+LATEX_HEADER: \setmainfont{Roboto}
#+LATEX_HEADER: \setbeamerfont{footnote}{size=\tiny}
#+LATEX_HEADER: \setbeamercolor{itemize item}{fg=accent}
#+LATEX_HEADER: \setbeamercolor{itemize subitem}{fg=accent}
#+LATEX_HEADER: \setbeamercolor{itemize subsubitem}{fg=accent}
#+LATEX_HEADER:  \setbeamertemplate{itemize item}[circle]
#+LATEX_HEADER:  \setbeamertemplate{itemize subitem}[square]
#+LATEX_HEADER:  \setbeamertemplate{itemize subsubitem}[triangle]
#+LATEX_HEADER: \makeatletter
#+LATEX_HEADER: \newsavebox{\mybox}
#+LATEX_HEADER: \setbeamertemplate{frametitle}{
#+LATEX_HEADER:  \nointerlineskip
#+LATEX_HEADER:  \savebox{\mybox}{%
#+LATEX_HEADER:      \begin{beamercolorbox}[%
#+LATEX_HEADER:          wd=\paperwidth,%
#+LATEX_HEADER:          sep=0pt,%
#+LATEX_HEADER:          ht=2.0ex,%
#+LATEX_HEADER:          dp=0.9ex,%
#+LATEX_HEADER:          leftskip=\metropolis@frametitle@padding,%
#+LATEX_HEADER:          rightskip=\metropolis@frametitle@padding,%
#+LATEX_HEADER:        ]{frametitle}%
#+LATEX_HEADER:      \metropolis@frametitlestrut@start\metropolis@frametitlestrut@end%
...
#+LATEX_HEADER:      rightskip=\metropolis@frametitle@padding,%
#+LATEX_HEADER:    ]{frametitle}%
#+LATEX_HEADER:  \metropolis@frametitlestrut@start\insertframetitle\metropolis@frametitlestrut@end%
#+LATEX_HEADER:  \end{beamercolorbox}%
#+LATEX_HEADER: }
#+LATEX_HEADER: \makeatother
#+LATEX_HEADER:  \makeatletter
#+LATEX_HEADER:  \pretocmd{\beamer@reseteecodes}{%
#+LATEX_HEADER:    \ifbool{metropolis@standout}{
#+LATEX_HEADER:      \endgroup
#+LATEX_HEADER:      \boolfalse{metropolis@standout}
#+LATEX_HEADER:    }{}
#+LATEX_HEADER:  }{}{}
#+LATEX_HEADER:  \makeatother
#+LATEX_HEADER:  \definecolor{accent}{HTML}{FDBD10}
#+LATEX_HEADER: \setbeamercolor{palette primary}{fg=accent, bg=black}
#+LATEX_HEADER:  \setbeamercolor{frametitle}{fg=accent, bg=black}
#+LATEX_HEADER:  \setbeamercolor{titlelike}{fg=black}
#+LATEX_HEADER:  \setbeamercolor{titlepage}{fg=black, bg=accent}
#+LATEX_HEADER:  \setbeamercolor{footnote}{bg=black}
#+LATEX_HEADER:  \setbeamercolor{normal text}{fg=black}
#+LATEX_HEADER:  \setbeamercolor{block title}{fg=black, bg=accent!40!white}
...
#+LATEX_HEADER:  \setbeamercolor{title separator}{fg = accent, bg=accent}
#+LATEX_HEADER: \setbeamercolor{progress bar}{bg=white, fg=accent}
#+LATEX_HEADER:  \setbeamercolor{progress bar in section page}{bg=white, fg=accent}
#+end_export
...
#+begin_src emacs-lisp :tangle init.el :results output none
...
#+begin_src emacs-lisp :results output none :tangle init.el
  (use-package org
    :ensure t
    :defer t
...
Prettier Org buffers are only half of the notebook experience — the notebook
also lets you *click* to run code. With =org-modern= handling the visuals, a
small font-lock layer makes =#+begin_src= headers and =:session= arguments
clickable: =mouse-1= on =#+begin_src= runs the block (and refreshes inline
images), and =mouse-1= on =:session= opens the block in the dedicated source
editor. Lifted from the maintainer's personal configuration, this replaces the
old =svg-tag-mode= badges (which needed a manual =C-c C-c= to load) with plain
text properties that work as soon as the buffer opens.
...
A few small Emacs packages that round out the scientific-writing workflow. They
are not on MELPA, so they are installed straight from their Git repositories
with =use-package='s built-in =:vc= support (Emacs 30+). On first start Emacs
clones each repo into =elpa/= and byte-compiles it; thereafter they load like
any other package.

- =org-poster= — build conference posters from a single Org file.
- =orcid-cv= — turn a public ORCID record into a LaTeX/PDF CV, in pure Emacs Lisp.
- =org-tracked-docx= — round-trip Org manuscripts to Word with tracked changes
  and CriticMarkup.
